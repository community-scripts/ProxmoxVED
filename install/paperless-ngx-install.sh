...
read -r -p "Would you like to use a custom PostgreSQL server? <y/N> " custom_pg_prompt
if [[ "${custom_pg_prompt,,}" =~ ^(y|yes)$ ]]; then
  read -r -p "Enter PostgreSQL Host: " DB_HOST
  read -r -p "Enter PostgreSQL Port: " DB_PORT
  read -r -p "Enter PostgreSQL Database Name: " DB_NAME
  read -r -p "Enter PostgreSQL Username: " DB_USER
  read -r -p "Enter PostgreSQL Password: " DB_PASS
else
  DB_HOST=localhost
  DB_PORT=5432
  DB_NAME=paperlessdb
...
echo -e "Paperless-ngx Database Host: \e[32m$DB_HOST\e[0m" >>~/paperless.creds
echo -e "Paperless-ngx Database Port: \e[32m$DB_PORT\e[0m" >>~/paperless.creds
...
if [[ ! "${custom_pg_prompt,,}" =~ ^(y|yes)$ ]]; then
  ...
  if [[ "${prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    echo -e "Adminer Server: \e[32m$DB_HOST:$DB_PORT\e[0m" >>~/paperless.creds
