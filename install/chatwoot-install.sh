...
ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY=$(echo "${ENCRYPT_OUTPUT}" | awk '/primary_key:/ {print $2}')
ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY=$(echo "${ENCRYPT_OUTPUT}" | awk '/deterministic_key:/ {print $2}')
ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT=$(echo "${ENCRYPT_OUTPUT}" | awk '/key_derivation_salt:/ {print $2}')
cat <<EOF >>/opt/chatwoot/.env
