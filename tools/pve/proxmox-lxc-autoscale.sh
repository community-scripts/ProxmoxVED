...
CONFIG_FILE="${INSTALL_DIR}/lxc_autoscale/lxc_autoscale.yaml"
...
install() {
  ...
  if [[ -d "$INSTALL_DIR" ]]; then
    ...
    systemctl stop lxc_autoscale.service 2>/dev/null || true
    systemctl disable lxc_autoscale.service 2>/dev/null || true
    [[ -f "$CONFIG_FILE" ]] && cp "$CONFIG_FILE" "/tmp/lxc_autoscale.yaml.bak"
    rm -rf "$INSTALL_DIR"
    msg_ok "Stopped existing service"
  fi

  fetch_and_deploy_gh_release "lxc_autoscale" "$REPO" "tarball" "latest" "$INSTALL_DIR"
  ...
  cat <<EOF >/etc/systemd/system/lxc_autoscale.service
[Unit]
...
ExecStart=/usr/local/bin/uv run python3 ${INSTALL_DIR}/lxc_autoscale/lxc_autoscale.py
Restart=always
RestartSec=30
...
EOF
  ...
  [[ -f "/tmp/lxc_autoscale.yaml.bak" ]] && cp "/tmp/lxc_autoscale.yaml.bak" "$CONFIG_FILE"

  msg_ok "${APP} installed successfully"
  echo -e "\nConfiguration: ${CONFIG_FILE}"
  echo -e "Service: systemctl status lxc_autoscale"
}
